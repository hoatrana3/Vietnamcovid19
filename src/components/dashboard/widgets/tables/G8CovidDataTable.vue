    ...
    export default {
        ...
        data() {
            return {
                ...
                g8Countries: ['CA', 'FR', 'DE', 'IT', 'JP', 'GB', 'US', 'RU'],
        ...
        mounted() {
            window.addEventListener(
                "orientationchange",
                () => {
                    DashboardUtils.customizeBStickyCols(document.getElementById('g8-covid-table'))
                }
            );
        ...
        methods: {
            getFormattedData(data) {
                const formattedData = _.clone(data).filter(country => this.g8Countries.includes(country.iso2));

                formattedData.unshift(this.calculateG8Data(formattedData));
            ...
            calculateG8Data(rawData) {
                const result = {
                    country: `${this.$t('places.all-g8-nations')} (${rawData.length})`,
                    country_vn: `${this.$t('places.all-g8-nations')} (${rawData.length})`,
            ...
            observeDataTable() {
                const tableDom = document.getElementById('g8-covid-table');
